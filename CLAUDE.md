...
| GET | `/api/admin/attendance/grid` | Loozer × trip attendance matrix. Returns `trips` (with `has_roster: bool`), `loozers`, `roster` rows (event_participants where on_roster=true), and `attendance` rows (event_attendance). Powers `/admin/attendance`. |
| PUT | `/api/admin/attendance/cell` | Body: `{userId, tripId, attended, target: "roster"\|"historical"}`. `target=roster` writes to `event_participants` (on_roster + likelihood=99 when checking; on_roster=false on uncheck, likelihood preserved). `target=historical` writes to `event_attendance`. Idempotent. |
...
- `00132_event_attendance.sql` - `event_attendance(user_id, trip_id, source)` table for Phase 1b of issue #114. Holds attendance for **historical (pre-app) events only** — sourced from the workbook importer at `/admin/history/import-attendance` (`source='workbook'`) or admin-marked via the `/admin/attendance` matrix for any trip without a live roster (`source='admin'`). For modern events, the live `event_participants.on_roster=true` set is the source of truth (driven by RSVPs and the per-event Roster admin). The lifetime "events attended" count surfaced on `/loozers` grid / family tree / profile page is a UNION of distinct `trip_id`s across both sources, so adding someone to the active event's roster (even before the event happens) bumps their count immediately.
