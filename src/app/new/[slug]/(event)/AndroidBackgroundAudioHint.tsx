...
const DISMISS_KEY = "v2_android_bg_audio_hint_dismissed";

// Visibility is derived from browser state (UA, install mode, a localStorage
// dismissal) — none of which exists during SSR. Read through useSyncExternalStore
// so the server snapshot is always false (no hydration flash) and dismiss() can
// notify subscribers without a setState-in-effect.
...
/**
 * Android-only, one-time tip that helps music keep playing when the screen is off
 * or the app is backgrounded — the companion to the MediaSession hardening in the
 * music provider. Nudges users toward installing the PWA + exempting it from
 * battery optimization (Samsung/Xiaomi/etc. kill background media aggressively).
 * Renders nothing on iOS/desktop, when already installed, or once dismissed.
 * (v2-local copy of the legacy hint.)
 */
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 flex-1">
          ...
          <ol className="mt-1.5 list-decimal space-y-1 pl-4 text-xs leading-relaxed text-green-800">
            ...
            <li>
              In Android Settings → Apps → your app → Battery, choose{" "}
