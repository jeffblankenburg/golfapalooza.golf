...
// ── Hole Scoring Cards (scores + BSPITW bonus combined) ──

function HoleScoringCards({
  teams,
  selectedTeamId,
  holes,
  holeScores,
  ...
  const { skinCounts, skinWins } = teams.length > 1
    ...
    : { skinCounts: new Map<string, number>(), skinWins: new Map<string, Set<number>>() };
  ...
  const handleScoreKeyDown = (e: React.KeyboardEvent<HTMLInputElement>, holeNumber: number) => {
    if (e.key === "Enter") {
      e.preventDefault();
      // Find the next hole's score input
      const nextInput = document.querySelector<HTMLInputElement>(
        `[data-score-hole="${holeNumber + 1}"]`
      );
      nextInput?.focus();
    }
  };
  ...
  return (
    <div className="space-y-2">
      ...
      {team.members.length > 0 && sortedHoles.map((hole) => {
        const strokes = teamScores[hole.hole_number];
        const holeBonuses = bonusPoints.filter(
          (b) => b.team_id === team.id && b.hole_number === hole.hole_number
        );
        const holedOutUserId = holeBonuses.find((b) => b.holed_out)?.user_id || null;
        const wonSkin = teamSkinHoles?.has(hole.hole_number);

        // Show nine subtotal after hole 9
        const showFrontSubtotal = hole.hole_number === 9;
        const showBackSubtotal = hole.hole_number === 18;

        return (
          <div key={hole.hole_number}>
            <div className={`bg-white rounded-xl border overflow-hidden ${wonSkin ? "border-green-400 ring-1 ring-green-300" : "border-gray-200"}`}>
              {/* Hole header with score input */}
            ...
            {/* Nine subtotals */}
            {showFrontSubtotal && (
              <div className="flex items-center justify-between px-3 py-1.5 mt-1 bg-gray-100 rounded-lg text-xs font-bold">
                <span className="text-gray-500">OUT</span>
                <span className={totalColor(out, parForNine(1, 9))}>{out ?? "—"}</span>
              </div>
            )}
            {showBackSubtotal && (
              <div className="flex items-center justify-between px-3 py-1.5 mt-1 bg-gray-100 rounded-lg text-xs font-bold">
                <span className="text-gray-500">IN</span>
                <span className={totalColor(inn, parForNine(10, 18))}>{inn ?? "—"}</span>
              </div>
            )}
          </div>
        );
