...
export default function CourseLookupModal({ initialName = "", onClose, onCourseReady, onManualFallback }: Props) {
  ...
  useEffect(() => {
    ...
    // Defer the first set out of the effect body (avoids the cascading-render lint).
    ...
    return () => { cancelAnimationFrame(raf); if (interval) clearInterval(interval); };
  }, [phase]);
  ...
  async function runLookup() {
    ...
    let res: Response;
    try {
      res = await fetch("/api/v2/courses/lookup", {
        method: "POST",
  ...
  async function commit(draft: Draft) {
    setCommitting(true);
    const res = await fetch("/api/v2/courses/lookup/commit", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  async function importAll() {
    ...
    setCommitting(true);
    setPhase("loading");
    const res = await fetch("/api/v2/courses/lookup/commit-bulk", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
